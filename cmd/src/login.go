...
`, endpointArg, endpointArg)
	...
	if cfg.ConfigFilePath != "" {
		fmt.Fprintln(out)
		fmt.Fprintf(out, "⚠️  Warning: Configuring src with a JSON file is deprecated. Please migrate to using the env vars SRC_ENDPOINT, SRC_ACCESS_TOKEN, and SRC_PROXY instead, and then remove %s. See https://github.com/khulnasoft/src-cli#readme for more information.\n", cfg.ConfigFilePath)
	}
	...
	if _, err := client.NewRequest(query, nil).Do(ctx, &result); err != nil {
		...
		fmt.Fprintln(out, createAccessTokenMessage)
		fmt.Fprintln(out, "   (If you need to supply custom HTTP request headers, see information about SRC_HEADER_* and SRC_HEADERS env vars at https://github.com/khulnasoft/src-cli/blob/main/AUTH_PROXY.md.)")
		return cmderrors.ExitCode1
	}
